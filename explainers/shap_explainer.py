...
class KernelShap(Explainer, name='kernel_shap'):
    ...
    def __init__(self, X_reference, predict_func, **kwargs):
        super().__init__()
        self.predict_func = predict_func
        self.X_reference = np.array(X_reference, dtype='float32')
        self.predict_func(self.X_reference[:3])  # just a test
        ...
        try:
            self.explainer = shap_lib.KernelExplainer(self.predict_func, self.X_reference, **kwargs)
    ...
    def explain(self, dataset_to_explain, **kwargs):
        ...
        shap_values = self.explainer.shap_values(np.array(dataset_to_explain, dtype=np.float32))
...
features are distributed according to ”the distribution generated by the tree”.>> See https://arxiv.org/pdf/1908.08474.pdf  I don t know if this still holds or was fixed. """
    ...
    def __init__(self, trained_model, X_reference, **kwargs):
        super().__init__()
        try:
            self.explainer = shap_lib.TreeExplainer(trained_model, X_reference, **kwargs)
        except Exception as e:
...
class Permutation(Explainer, name='permutation'):
    ...
    def __init__(self, predict_func, X_reference, input_features, **kwargs):  # todo change input_features to len(inputs
        super().__init__()
        ...
        self.explainer = shap_lib.explainers.Permutation(predict_func, masker, max_evals=max(500, 2 * (len(input_features) + 1)))
...
class PermutationPartition(Explainer):
    def __init__(self, predict_func, X_reference, input_features, **kwargs):  # todo change input_features to its length
        masker = shap_lib.maskers.Partition(X_reference)
        self.explainer = shap_lib.explainers.Permutation(predict_func, masker, max_evals=max(500, 2 * (len(input_features) + 1)))
